...
COMMANDS = ['yum list installed git || yum install git',
            'yum list installed epel-release || yum install epel-release',
            'test -d /tmp/openshift-ansible || ' \
            'git clone https://github.com/openshift/openshift-ansible.git /tmp/openshift-ansible',
            'yum list installed ansible || yum install ansible',
            'yum list installed pyOpenSSL ||yum install pyOpenSSL',
    ...
    Net::SSH.start($evm.root['deployment_master'], $evm.root['user'], :paranoid => false, :forward_agent => true,
      ...
      COMMANDS.each do |cmd|
        ...
        unless result[:exit_code] == 0
          $evm.root['automation_task'].message = "FAILED: couldn't execute command " + cmd
