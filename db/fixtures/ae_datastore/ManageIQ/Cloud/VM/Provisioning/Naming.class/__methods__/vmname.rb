...
vm_name = nil if vm_name.blank? or vm_name == 'changeme'
...
def get_env_tag(prov)
  ...
  return env[0, 3] if not env.blank?
end
...
def get_suffix(prov, condensed)
  ...
  return "$n{3}" if vm_num > 1 or not condensed
end

# If the name is already specified, use it and exit
if not vm_name.blank?
...
$evm.log("info", "VM Name: <#{$evm.object['vmname']}>")
