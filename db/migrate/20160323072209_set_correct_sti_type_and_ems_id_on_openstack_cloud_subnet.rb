class SetCorrectStiTypeAndEmsIdOnOpenstackCloudSubnet< ActiveRecord::Migration[5.0]
  ...
  def up
    ...
    ExtManagementSystem
      .where({:type => 'ManageIQ::Providers::Openstack::NetworkManager'}).each do |ems|
      CloudSubnet
        .joins('left join cloud_networks on cloud_networks.id = cloud_subnets.cloud_network_id')
        .where(:cloud_subnets  => {:type   => ['ManageIQ::Providers::Openstack::CloudManager::CloudSubnet',
                                               'ManageIQ::Providers::Openstack::InfraManager::CloudSubnet']},
               :cloud_networks => {:ems_id => ems.id})
        .update_all("type = 'ManageIQ::Providers::Openstack::NetworkManager::CloudSubnet', ems_id = '#{ems.id}'")
    end
  end
  ...
  def down
    ...
    CloudSubnet
      ...
      .where(:cloud_subnets => {:type => 'ManageIQ::Providers::Openstack::NetworkManager::CloudSubnet'},
             :cloud_manager => {:type => 'ManageIQ::Providers::Openstack::CloudManager'})
      .update_all("type = 'ManageIQ::Providers::Openstack::CloudManager::CloudSubnet'")
    ...
    CloudSubnet
      ...
      .joins('left join ext_management_systems as cloud_manager on ext_management_systems.parent_ems_id = cloud_manager.id')
      .where(:cloud_subnets => {:type => 'ManageIQ::Providers::Openstack::NetworkManager::CloudSubnet'},
             :cloud_manager => {:type => 'ManageIQ::Providers::Openstack::InfraManager'})
      .update_all("type = 'ManageIQ::Providers::Openstack::InfraManager::CloudSubnet'")
  end
end
