...
class VmwareWinWorkStationServer
  def RegisteredVmNames
    ...
    vm_list = {}
    get_user_app_data_paths.each do |d|
      if File.directory?(d)
        f = File.join(d, vmware_data_path, "favorites.vmls")
        VmConfig.to_h(f).each { |_k, v| vm_list[v[:config].downcase] ||= v } if File.exist?(f)
      end
    end

    # Make sure we do not return any VMs that do not exist anymore
    vm_list.delete_if { |_key, value| !File.exist?(value[:config]) }

    vm_list.collect { |_k, v| v[:config] }
  end
