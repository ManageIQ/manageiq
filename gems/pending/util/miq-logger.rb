...
if $log.nil?
  ...
  class MIQLogger
    def self.get_log(config, fileName = nil, logName = "toplog")
      ...
      if log.nil?
        ...
        unless fileName.nil?
          ...
          logConfig = {
            :filename  => File.join(logSettings[:logDir], getLogName(fileName, config)),
    ...
    def self.cleanup_logs(targetDir, basename, miqhost_keep, log)
      files = collect_filenames(targetDir, basename, log)
    ...
    class Logger
      ...
      def miqLogHeader(fileName = nil, logData = true)
        ...
        if logData == true && @miqFileName.nil? == false
          ...
          summary init_msg
          summary border
          summary "Current log level:[#{Log4r::LNAMES[level]}]"
        end
      end
    end

    class RollingFileOutputter
      alias_method :roll_old, :roll
