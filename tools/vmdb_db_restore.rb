...
def run_cmd(msg, env_or_cmd, cmd = nil)
  ...
  unless cmd.include?("pg_restore")
    raise("Error:  #{cmd} did not complete sucessfully!\n\n#{outdata.join("")}") unless status.success?
  end
ensure
  STDIN.echo = true
end
...
def import_db
  if File.open(@db_dump_file, "rb") { |f| f.readpartial(5) } == "PGDMP"
    run_cmd("Dumping data into #{@db_name}", %(pg_restore -v -U root -j 4 -d #{@db_name} "#{@db_dump_file}"))
  else
    stdin_proc = proc do |stdin|
      require 'zlib'
    ...
    run_cmd("Dumping data into #{@db_name}", {}, %(psql -U root -d #{@db_name}), &stdin_proc)
  end
end

run_cmd("Bundle update", %(bin/bundle update))
run_cmd("Creating database", cmd_env, migrate_database)
import_db
run_cmd("Migrating database", cmd_env, %(bin/rake db:migrate))
run_cmd("Fixing database auth",          %(bundle exec tools/fix_auth.rb --v2 --invalid bogus --db #{@db_name}))
run_cmd("Update dependencies",  cmd_env, %(bin/update))
run_cmd("Update default auth",  cmd_env, %[bin/rails runner 'User.where(:userid => "admin").each {|u| u.update_attribute :password, "smartvm"}'])
