describe "Available_CloudSubnets Method Validation" do
  let(:user) { FactoryGirl.create(:user_with_group) }
  before do
    @ins = "/Cloud/LoadBalancer/Operations/Amazon/Methods/Available_CloudSubnets"
  end
  ...
  before :each do
    ...
    @cloud_network_2 = FactoryGirl.create(:cloud_network_amazon,
                                          :name                  => 'net 2',
                                          :ext_management_system => ems)
    @cloud_subnet_1  = FactoryGirl.create(:cloud_subnet_amazon,
                                          :name                  => 'subnet 1',
                                          :cloud_network         => @cloud_network_1,
                                          :ext_management_system => ems)
    @cloud_subnet_2  = FactoryGirl.create(:cloud_subnet_amazon,
                                          :name                  => 'subnet 2',
                                          :cloud_network         => @cloud_network_1,
                                          :ext_management_system => ems)
  end
  ...
  context "workspace has a cloud_network without cloud_subnets" do
    let(:service_template) do
      FactoryGirl.create(:service_template_load_balancer, :load_balancer_manager => ems)
    end
  ...
  context "workspace has load_balancer service template" do
    ...
    it "finds all the cloud_networks and populates the list" do
      ...
      expect(ws.root["values"]).to include(
        @cloud_subnet_1.id => @cloud_subnet_1.name,
        @cloud_subnet_2.id => @cloud_subnet_2.name
      )
      expect(ws.root["default_value"]).to be_nil
    end
