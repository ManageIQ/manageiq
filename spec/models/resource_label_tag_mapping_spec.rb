describe ResourceLabelTagMapping do
  ...
  def map_labels(model_name, labels_kv)
    ResourceLabelTagMapping.map_labels(ResourceLabelTagMapping.cache,
                                        model_name,
                                        labels(labels_kv))
  end

  def to_tags(tag_hashes)
    tag_hashes.map { |h| ResourceLabelTagMapping.find_or_create_tag(h) }
  end
  ...
  context "with empty mapping" do
    it "does nothing" do
      expect(ResourceLabelTagMapping.cache).to be_empty
  ...
  context "with 2 mappings for same label" do
    before(:each) do
      FactoryGirl.create(:resource_label_tag_mapping, :only_nodes, :label_value => 'value-1', :tag => tag1)
      FactoryGirl.create(:resource_label_tag_mapping, :only_nodes, :label_value => 'value-1', :tag => tag2)
    end
  ...
  context "with any-value and specific-value mappings" do
    before(:each) do
      FactoryGirl.create(:resource_label_tag_mapping, :tag => cat_tag)
      FactoryGirl.create(:resource_label_tag_mapping, :label_value => 'value-1', :tag => tag1)
      FactoryGirl.create(:resource_label_tag_mapping, :label_value => 'value-1', :tag => tag2)
    ...
    it "creates tag for new value" do
      expect(ResourceLabelTagMapping.controls_tag?(tag1)).to be true
      expect(ResourceLabelTagMapping.controls_tag?(tag2)).to be true
      expect(ResourceLabelTagMapping.controls_tag?(tag_in_another_cat)).to be false

      cached1 = ResourceLabelTagMapping.cache
      tags = to_tags(ResourceLabelTagMapping.map_labels(cached1, 'ContainerNode', labels('name' => 'value-2')))
      ...
      expect(ResourceLabelTagMapping.controls_tag?(tag1)).to be true
      expect(ResourceLabelTagMapping.controls_tag?(tag2)).to be true
      expect(ResourceLabelTagMapping.controls_tag?(tags[0])).to be true

      # But nothing changes when called again, the previously created tag is re-used.

      tags2 = to_tags(ResourceLabelTagMapping.map_labels(cached1, 'ContainerNode', labels('name' => 'value-2')))
      ...
      cached2 = ResourceLabelTagMapping.cache

      tags2 = to_tags(ResourceLabelTagMapping.map_labels(cached2, 'ContainerNode', labels('name' => 'value-2')))
      expect(tags2).to contain_exactly(tags[0])
    ...
    it "handles names that differ only by case" do
      ...
      FactoryGirl.create(:resource_label_tag_mapping,
    ...
    context "2 workers with independent cache" do
      it "handle known value simultaneously" do
        cached1 = ResourceLabelTagMapping.cache
        cached2 = ResourceLabelTagMapping.cache
        tags1 = to_tags(ResourceLabelTagMapping.map_labels(cached1, 'ContainerNode', labels('name' => 'value-1')))
        tags2 = to_tags(ResourceLabelTagMapping.map_labels(cached2, 'ContainerNode', labels('name' => 'value-1')))
      ...
      it "handle new value encountered simultaneously" do
        cached1 = ResourceLabelTagMapping.cache
        cached2 = ResourceLabelTagMapping.cache
        tags1 = to_tags(ResourceLabelTagMapping.map_labels(cached1, 'ContainerNode', labels('name' => 'value-2')))
        tags2 = to_tags(ResourceLabelTagMapping.map_labels(cached2, 'ContainerNode', labels('name' => 'value-2')))
  ...
  context "with 2 any-value mappings onto same category" do
    before(:each) do
      FactoryGirl.create(:resource_label_tag_mapping, :label_name => 'name1', :tag => cat_tag)
      FactoryGirl.create(:resource_label_tag_mapping, :label_name => 'name2', :tag => cat_tag)
    end
  ...
  context "given a label with empty value" do
    ...
    it "honors specific mapping for the empty value" do
      FactoryGirl.create(:resource_label_tag_mapping, :label_value => '', :tag => empty_tag_under_cat)
  ...
  context "with any-type and specific-type mappings" do
    before(:each) do
      FactoryGirl.create(:resource_label_tag_mapping, :only_nodes, :label_value => 'value', :tag => tag1)
      FactoryGirl.create(:resource_label_tag_mapping, :label_value => 'value', :tag => tag2)
    end
  ...
  context "any-type specific-value vs specific-type any-value" do
    before(:each) do
      FactoryGirl.create(:resource_label_tag_mapping, :only_nodes, :tag => cat_tag)
      FactoryGirl.create(:resource_label_tag_mapping, :label_value => 'value', :tag => tag2)
    end
  ...
  describe ".retag_entity" do
    ...
    it "assigns new tags, idempotently" do
      ...
      ResourceLabelTagMapping.retag_entity(node, [{:tag_id => tag1.id}])
    ...
    it "unassigns obsolete mapping-controlled tags" do
      node.tags = [tag1]
      ResourceLabelTagMapping.retag_entity(node, [])
      expect(node.tags).to be_empty
    end

    it "preserves user tags" do
      user_tag = FactoryGirl.create(:tag, :name => '/managed/mycat/mytag')
      expect(ResourceLabelTagMapping.controls_tag?(user_tag)).to be false
      expect(ResourceLabelTagMapping.controls_tag?(tag1)).to be true
      expect(ResourceLabelTagMapping.controls_tag?(tag2)).to be true
      expect(ResourceLabelTagMapping.controls_tag?(tag3)).to be true
      node.tags = [tag1, user_tag, tag2]
      ResourceLabelTagMapping.retag_entity(node, [{:tag_id => tag1.id}, {:tag_id => tag3.id}])
