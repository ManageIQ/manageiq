  ...
  context "ip checks" do
    ...
    context "container deployment nodes" do
      ...
      it "create deployment nodes works properly" do
        ...
        @container_deployment.create_deployment_nodes([[{"vmName" => "10.0.0.1"}, {"vmName" => "example.com"}], [], [{"vmName" => "37.142.68.50"}]], nil, ["node", "master", "deployment_master"])
        expect(@container_deployment.nodes.count).to eql(2)
        expect(@container_deployment.masters.count).to eql(0)
        expect(@container_deployment.deployment_master).to be_truthy
      end
    end

    context "ansible cfg" do
      it "checks generate_ansible_yaml returns correct yaml" do
        expect(@container_deployment.generate_ansible_yaml).to eql(:version    => "v3",
                                                                   :deployment => {:ansible_config   => "/usr/share/atomic-openshift-utils/ansible.cfg",
