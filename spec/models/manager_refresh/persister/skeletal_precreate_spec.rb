...
describe ManagerRefresh::Inventory::Persister do
  ...
  it "tests smart batching batches only records with the same fields" do
    ...
    persister.container_projects.build(container_project_data(1).except(:name))
    persister.container_projects.build(container_project_data(2))
    ...
    # The batch saving must batch together only hashes with the same keys, to avoid nullifying unexpected values
  ...
  it "lazy_find with secondary ref doesn't pre-create records but finds them in DB, even when disconnected" do
    # TODO(lsmola) we can't find disconnected records now, we should
  ...
  it "pre-create doesn't shadow local db strategy" do
    # TODO(lsmola) it does now, so if there is a lazy_find for object, then lazy_find with key, the key will result to
    # nil, since the pre-created object doesn't have it. Can we fix it? The scope of model needs to be all, for db
    # finders
    FactoryGirl.create(:container_project, container_project_data(1).merge(:ems_id => @ems.id))
    ...
    persister.container_groups.build(
      container_group_data(
        1,
        :container_project => lazy_find_container_project,
        # This should go from a DB rather than skeletal pre-create
