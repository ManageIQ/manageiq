...
describe UserGroup do
  describe "::seed" do
    let(:role) { described_class.first }
    ...
    it "creates UserGroup with Entitlement by MiqUserRole" do
      ...
      expect(role.description).to eq("EvmGroup-super_administrator")
      expect(role.miq_groups.first).to be_present
      expect(role.miq_groups.first.miq_user_role).to be_present
      expect(role.miq_groups.first.miq_user_role.name).to eq miq_user_role.name
    end
