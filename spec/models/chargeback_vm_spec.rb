    ...
    context 'with metric rollups' do
      ...
      context 'monthly report, group by tenants' do
        ...
        it 'calculates allocated,used metric with using max,avg method with vcpus=1.0 and 50% usage' do
          expect(subject_row_for_tenant(tenant_1).cpu_allocated_metric).to eq(1)
          expect(subject_row_for_tenant(tenant_1).cpu_allocated_cost).to eq(1 * monthly_allocated_rate)

          expect(subject_row_for_tenant(tenant_2).cpu_allocated_metric).to eq(1)
          expect(subject_row_for_tenant(tenant_2).cpu_allocated_cost).to eq(1 * monthly_allocated_rate)

          expect(subject_row_for_tenant(tenant_3).cpu_allocated_metric).to eq(1)
          expect(subject_row_for_tenant(tenant_3).cpu_allocated_cost).to eq(1 * monthly_allocated_rate)

          expect(subject_row_for_tenant(tenant_4).cpu_allocated_metric).to eq(1)
          expect(subject_row_for_tenant(tenant_4).cpu_allocated_cost).to eq(1 * monthly_allocated_rate)

          expect(subject_row_for_tenant(tenant_5).cpu_allocated_metric).to eq(1)
          expect(subject_row_for_tenant(tenant_5).cpu_allocated_cost).to eq(1 * monthly_allocated_rate)
          ...
          expect(subject_row_for_tenant(tenant_1).cpu_used_metric).to eq(5 * 2 * 50 * hourly_usage)
          # and cost - there is multiplication by monthly_used_rate
          expect(subject_row_for_tenant(tenant_1).cpu_used_cost).to eq(5 * 2 * 50 * hourly_usage * monthly_used_rate)

          expect(subject_row_for_tenant(tenant_2).cpu_used_metric).to eq(1 * 2 * 50 * hourly_usage)
          expect(subject_row_for_tenant(tenant_2).cpu_used_cost).to eq(1 * 2 * 50 * hourly_usage * monthly_used_rate)

          expect(subject_row_for_tenant(tenant_3).cpu_used_metric).to eq(3 * 2 * 50 * hourly_usage)
          expect(subject_row_for_tenant(tenant_3).cpu_used_cost).to eq(3 * 2 * 50 * hourly_usage * monthly_used_rate)

          expect(subject_row_for_tenant(tenant_4).cpu_used_metric).to eq(1 * 2 * 50 * hourly_usage)
          expect(subject_row_for_tenant(tenant_4).cpu_used_cost).to eq(1 * 2 * 50 * hourly_usage * monthly_used_rate)

          expect(subject_row_for_tenant(tenant_5).cpu_used_metric).to eq(1 * 2 * 50 * hourly_usage)
          expect(subject_row_for_tenant(tenant_5).cpu_used_cost).to eq(1 * 2 * 50 * hourly_usage * monthly_used_rate)
        end
        ...
        context 'vcpu=5 for VMs of tenant_5' do
          ...
          it 'calculates allocated,used metric with using max,avg method with vcpus=1.0 and 50% usage' do
            expect(subject_row_for_tenant(tenant_1).cpu_allocated_metric).to eq(5)
            expect(subject_row_for_tenant(tenant_1).cpu_allocated_cost).to eq(5 * monthly_allocated_rate)

            expect(subject_row_for_tenant(tenant_2).cpu_allocated_metric).to eq(1)
            expect(subject_row_for_tenant(tenant_2).cpu_allocated_cost).to eq(1 * monthly_allocated_rate)

            expect(subject_row_for_tenant(tenant_3).cpu_allocated_metric).to eq(5)
            expect(subject_row_for_tenant(tenant_3).cpu_allocated_cost).to eq(5 * monthly_allocated_rate)

            expect(subject_row_for_tenant(tenant_4).cpu_allocated_metric).to eq(1)
            expect(subject_row_for_tenant(tenant_4).cpu_allocated_cost).to eq(1 * monthly_allocated_rate)

            expect(subject_row_for_tenant(tenant_5).cpu_allocated_metric).to eq(5)
            expect(subject_row_for_tenant(tenant_5).cpu_allocated_cost).to eq(5 * monthly_allocated_rate)

            # each tenant has 2 VMs and each VM  has 50 of cpu usage:
            # 5 tenants(tenant_1 has 4 tenants and plus tenant_1 ) * 2 VMs * 50% of usage
            # but tenant_5 has  2 VMs and each VM  has 75 of cpu usage
            expect(subject_row_for_tenant(tenant_1).cpu_used_metric).to eq(hourly_usage * (4 * 2 * 50 + 1 * 2 * 75))
            # and cost - there is multiplication by  monthly_used_rate
            expect(subject_row_for_tenant(tenant_1).cpu_used_cost).to eq(hourly_usage * (4 * 2 * 50 + 1 * 2 * 75) * monthly_used_rate)

            expect(subject_row_for_tenant(tenant_2).cpu_used_metric).to eq(hourly_usage * 1 * 2 * 50)
            expect(subject_row_for_tenant(tenant_2).cpu_used_cost).to eq(hourly_usage * 1 * 2 * 50 * monthly_used_rate)

            expect(subject_row_for_tenant(tenant_3).cpu_used_metric).to eq(hourly_usage * (2 * 2 * 50 + 1 * 2 * 75))
            expect(subject_row_for_tenant(tenant_3).cpu_used_cost).to eq(hourly_usage * (2 * 2 * 50 + 1 * 2 * 75) * monthly_used_rate)

            expect(subject_row_for_tenant(tenant_4).cpu_used_metric).to eq(hourly_usage * 1 * 2 * 50)
            expect(subject_row_for_tenant(tenant_4).cpu_used_cost).to eq(hourly_usage * 1 * 2 * 50 * monthly_used_rate)

            expect(subject_row_for_tenant(tenant_5).cpu_used_metric).to eq(hourly_usage * 1 * 2 * 75)
            expect(subject_row_for_tenant(tenant_5).cpu_used_cost).to eq(hourly_usage * 1 * 2 * 75 * monthly_used_rate)
          end
          ...
          context 'test against group by vm report' do
            ...
            it 'calculates used metric and cost same as report for each vm' do
              # Tenant 1 VMs
              all_vms_cpu_metric = [vm_1_1, vm_2_1, vm_1_2, vm_2_2, vm_1_3, vm_2_3, vm_1_4, vm_2_4, vm_1_5, vm_2_5].map { |vm| result_row_for_vm(vm).cpu_used_metric }.sum
              all_vms_cpu_cost   = [vm_1_1, vm_2_1, vm_1_2, vm_2_2, vm_1_3, vm_2_3, vm_1_4, vm_2_4, vm_1_5, vm_2_5].map { |vm| result_row_for_vm(vm).cpu_used_cost }.sum
            ...
            it 'calculated allocted metric and cost with using max(max is not summed up - it is taken maximum)' do
              # Tenant 1 VMs
              all_vms_cpu_metric = [vm_1_1, vm_2_1, vm_1_2, vm_2_2, vm_1_3, vm_2_3, vm_1_4, vm_2_4, vm_1_5, vm_2_5].map { |vm| result_row_for_vm(vm).cpu_allocated_metric }.max
              all_vms_cpu_cost   = [vm_1_1, vm_2_1, vm_1_2, vm_2_2, vm_1_3, vm_2_3, vm_1_4, vm_2_4, vm_1_5, vm_2_5].map { |vm| result_row_for_vm(vm).cpu_allocated_cost }.max
