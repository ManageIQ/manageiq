  ...
  context "instance" do
    ...
    context "with a worker" do
      before do
        ...
        allow(@miq_server.worker_manager).to receive(:validate_worker).and_return(true)
        @miq_server.worker_manager.worker_add(@worker.pid)
      end
      ...
      it "quiesce will update status to quiesce, deactivate_roles, quiesce workers, clean active messages, and set status to stopped" do
        ...
        expect(@miq_server.worker_manager).to receive(:quiesce_workers_loop)
      ...
      it "quiesce_workers_loop will initiate shutdown of workers" do
        expect(@miq_server.worker_manager).to receive(:stop_worker)
        @miq_server.worker_manager.instance_variable_set(:@worker_monitor_settings, :quiesce_loop_timeout => 15.minutes)
        expect(@miq_server.worker_manager).to receive(:workers_quiesced?).and_return(true)
        @miq_server.worker_manager.quiesce_workers_loop
      end
      ...
      it "quiesce_workers_loop_timeout? will return true if timeout reached" do
        @miq_server.worker_manager.instance_variable_set(:@quiesce_started_on, Time.now.utc)
        @miq_server.worker_manager.instance_variable_set(:@quiesce_loop_timeout, 10.minutes)
        expect(@miq_server.worker_manager.quiesce_workers_loop_timeout?).not_to be_truthy

        Timecop.travel 10.minutes do
          expect(@miq_server.worker_manager.quiesce_workers_loop_timeout?).to be_truthy
        end
      end
