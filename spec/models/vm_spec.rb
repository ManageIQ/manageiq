describe Vm do
  ...
  describe ".user_or_group_owned" do
    ...
    context "by user in this region" do
      it "returns vm owned by user" do
        vm.evm_owner = owning_user
        vm.save!

        expect(described_class.user_or_group_owned(owning_user, nil)).to eq([vm])
      end

      it "returns vm owned by user or group" do
        vm.evm_owner = owning_user
        vm.save!

        expect(described_class.user_or_group_owned(owning_user, owning_user.current_group)).to eq([vm])
      end
    end
    ...
    context "by user in a remote region" do
      it "returns vm owned by user" do
        ...
        remote_owning_user = FactoryGirl.create :user, :id => remote_id, :userid => "user_owner"
        vm.id = remote_id
        vm.evm_owner = remote_owning_user
        vm.save!

        expect(vm.evm_owner_id).not_to eq(owning_user.id)
        expect(described_class.user_or_group_owned(owning_user, nil)).to eq([vm])
      end
