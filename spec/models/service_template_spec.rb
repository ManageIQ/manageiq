describe ServiceTemplate do
  ...
  context "#order" do
    ...
    context "#provision_request" do
      ...
      let(:resource_action_workflow) { ResourceActionWorkflow.new(arg1, user, resource_action, hash) }

      it "provisions a service template without errors" do
        expect(ResourceActionWorkflow).to(receive(:new).with(arg1, user, resource_action, hash).and_return(resource_action_workflow))
        expect(resource_action_workflow).to receive(:validate_dialog).and_return(nil)
