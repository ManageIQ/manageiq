describe ManageIQ::Providers::EmbeddedAnsible::AutomationManager::Refresher do
  ...
  # ruby -pi -e 'gsub /yourdomain.com/, "example.com"; gsub /admin:smartvm/, "testuser:secret"' spec/vcr_cassettes/manageiq/providers/embedded_ansible/automation_manager/refresher_v2.yml
  # replace with your working credentials
  # ruby -pi -e 'gsub /example.com/, "yourdomain.com"; gsub /testuser:secret/, "admin:smartvm"' spec/vcr_cassettes/manageiq/providers/embedded_ansible/automation_manager/refresher_v2.yml
  ...
  let(:provider) do
    ...
    FactoryGirl.create(:provider_embedded_ansible,
                       :zone       => zone,
                       :url        => tower_url,
  ...
  it ".ems_type" do
    expect(described_class.ems_type).to eq(:embedded_ansible_automation)
  end
  ...
  def assert_playbooks
    expect(expected_configuration_script_source.configuration_script_payloads.first).to be_an_instance_of(ManageIQ::Providers::EmbeddedAnsible::AutomationManager::Playbook)
  ...
  def assert_configuration_script_sources
    expect(automation_manager.configuration_script_sources.count).to eq(6)
    expect(expected_configuration_script_source).to be_an_instance_of(ManageIQ::Providers::EmbeddedAnsible::AutomationManager::ConfigurationScriptSource)
  ...
  def assert_configured_system
    expect(expected_configured_system).to have_attributes(
      :type                 => "ManageIQ::Providers::EmbeddedAnsible::AutomationManager::ConfiguredSystem",
