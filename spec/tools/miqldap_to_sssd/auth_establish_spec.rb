...
describe MiqLdapToSssd::AuthEstablish do
  describe '#run_auth_establish' do
    ...
    context "when authselect is available" do
      before do
        allow_any_instance_of(described_class).to receive(:authselect_found?).and_return(true)
      end
      ...
      it 'invokes authconfig with valid parameters' do
        ...
        described_class.new(@initial_settings).run_auth_establish
      end
      ...
      it 'handles authconfig failures' do
        ...
        expect { described_class.new(@initial_settings).run_auth_establish }.to raise_error(MiqLdapToSssd::AuthEstablishError)
      end
    end

    context "when authselect is not available" do
      before do
        allow_any_instance_of(described_class).to receive(:authselect_found?).and_return(false)
      end
