describe ProviderForemanController do
  ...
  context "renders tree_select" do
    ...
    it "renders the list view based on the nodetype(root,provider,config_profile) and the search associated with it" do
      ...
      controller.instance_variable_set(:@_params, :id => ems_id)
      controller.send(:tree_select)
      gtl_init_data = controller.init_report_data('miqGtlContoller')
    ...
    it "renders tree_select for ansible tower job templates tree node" do
      ...
      gtl_init_data = controller.init_report_data('miqGtlContoller')
      expect(gtl_init_data[:data][:modelName]).to eq("manageiq/providers/configuration_managers")
      expect(gtl_init_data[:data][:activeTree]).to eq("configuration_scripts_tree")
      expect(gtl_init_data[:data][:currId]).to eq("at-" + ApplicationRecord.compress_id(@config_ans.id))
      expect(gtl_init_data[:data][:isExplorer]).to eq(true)
