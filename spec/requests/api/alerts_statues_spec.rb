describe "Alert Status API" do
  ...
  it "get all alerts statuses by container provider" do
    ...
    expect(first_provider["alerts_types"]["danger"]["alerts"].count).to eq(0)
    expect(first_provider["alerts_types"]["warning"]["alerts"].count).to eq(0)
    expect(first_provider["alerts_types"]["info"]["alerts"].count).to eq(1)
    expect(first_provider["alerts_types"]["info"]["alerts"].first["description"]).to eq(miq_alert.description)
    expect(first_provider["alerts_types"]["info"]["alerts"].first["asignee_name"]).to eq(user.name)
  end
