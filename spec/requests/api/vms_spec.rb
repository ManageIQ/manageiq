...
describe "Vms API" do
  ...
  context 'Vm assign_tags action' do
    ...
    it 'can bulk assign tags to multiple VMs' do
      ...
      expected = {
        'results' => [
          { vm1.id.to_s => [a_hash_including('success'      => true,
                                             'tag_category' => tag1[:category],
                                             'tag_name'     => tag1[:name])]},
          { vm2.id.to_s => [a_hash_including('success'      => true,
    ...
    it 'will return success and failure messages for each resource and tag' do
      ...
      expected = {
        'results' => [
          { "999999" => a_hash_including('success' => false, 'message' => a_string_including("Couldn't find Vm")) },
          { vm2.id.to_s => [a_hash_including('success'      => false,
