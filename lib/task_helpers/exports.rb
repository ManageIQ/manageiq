module TaskHelpers
  class Exports
	def get_tmp_filename(object)
	  ...
	  elsif object.class == Hash then
	   # CustomizationTemplate Hash
	   if object[:class].include? "CustomizationTemplate" then	     
		   ...
		   elsif object[:class].include? "MiqDialog" then
		   tmp_filename = "#{object[:dialog_type]}-#{object[:name]}"
		   # Role Hash
	       elsif object[:class].include? "MiqUserRole" then
		   tmp_filename = role_hash[:name]
		   # Service Dialog Hash
		   elsif object[:class].include? "Dialog" then
    ...
    def self.safe_filename(object, keep_spaces = false, super_safe_filename = false)
	  ...
	  if super_safe_filename then
	    # Generate filename by id. Hashes and Miq classes must be handled.
		if object.class == Hash then
			new_filename = object[:id]
			else
			new_filename = object.id
		end
