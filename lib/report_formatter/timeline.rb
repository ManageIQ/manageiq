    ...
    def tl_event(tl_xml, row, col)
      ...
      unless rec.nil?
        ...
        when "BottleneckEvent"
          ...
          e_image = "/images/100/#{bubble_icon(rec[:resource_type])}.png"
          e_icon = "/images/timeline/#{rec.event_type.downcase}_#{rec[:severity]}.png"
        #         e_text = e_title # Commented out since name is showing in the columns anyway
        when "Vm"
          e_title = rec[:name]
          e_icon = "/images/timeline/vendor-#{rec.vendor.downcase}.png"
          e_image = "/images/100/os-#{rec.os_image_name.downcase}.png"
          e_text = "&lt;a href='/vm/show/#{rec.id}'&gt;#{e_title}&lt;/a&gt;"
        when "Host"
          e_title = rec[:name]
          e_icon = "/images/timeline/vendor-#{rec.vmm_vendor.downcase}.png"
          e_image = "/images/100/os-#{rec.os_image_name.downcase}.png"
        ...
        when "EventStream"
          ...
          if rec.event_type == "GeneralUserEvent"
            if rec.message.include?("EVM SmartState Analysis")
              e_icon =  "/images/timeline/evm_analysis.png"
            end
          end
          if rec[:vm_or_template_id] && Vm.exists?(rec[:vm_or_template_id])
            e_image = "/images/100/os-#{Vm.find(rec[:vm_or_template_id]).os_image_name.downcase}.png"
          end
          e_text = e_title
        when "PolicyEvent"
