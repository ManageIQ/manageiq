/*
 * Ruby module bridge to VixDiskLib 1.1.
 */
...
static char *exception_name = "VixDiskLibError_raw";
...
vdl_read(VALUE rSelf, VALUE rDiskHandle, VALUE rStartSector, VALUE rNumSectors) {
  ...
  RSTRING(rReadBuffer)->len = buflen;
  vReadBuffer = RSTRING(rReadBuffer)->ptr;
...
vdl_readMetadata(VALUE rSelf, VALUE rDiskHandle, VALUE rKey) {
  ...
  RSTRING(rValue)->len = strlen(vValue);
  return rValue;
}
...
vdl_write(VALUE rSelf, VALUE rDiskHandle, VALUE rStartSector, VALUE rNumSectors, VALUE rWriteBuffer) {
  ...
  if ( len > RSTRING_LEN(StringValue(rWriteBuffer)))   {
