/*
 * Ruby module bridge to VixDiskLib 5.0.
 */
...
static VALUE thumbPrint_sym;
static VALUE privateUse_sym;
...
/*
 * The "isAttachPossible" class method.
 */
static VALUE
vdl_isAttachPossible(VALUE rSelf, VALUE rParentHandle, VALUE rChildHandle)   {
  ...
  rb_log(LOGLEVEL_VERBOSE, "%s.isAttachPossible: status=[starting] parentDiskHandle=[0x%x], childDiskHandle=[0x%x]", class_name, *pvParentDiskHandle, *pvChildDiskHandle);
  vixError = VixDiskLib_IsAttachPossible(*pvParentDiskHandle, *pvChildDiskHandle);
  rValue = VIX_SUCCEEDED(vixError) ? Qtrue : Qfalse;
  rb_log(LOGLEVEL_VERBOSE, "%s.isAttachPossible: status=[complete] parentDiskHandle=[0x%x], childDiskHandle=[0x%x] isAttachPossible=[%s]", class_name, *pvParentDiskHandle, *pvChildDiskHandle, VIX_SUCCEEDED(vixError) ? "true" : "false");
...
/*
 * The "prepareForAccess" class method.
 */
static VALUE
vdl_prepareForAccess(VALUE rSelf, VALUE rConnectParams, VALUE rIdentity) {
  VixDiskLibConnectParams vConnectParams;
  VixError                vixError;
  VALUE                   tval;
  const char             *vIdentity;
  ...
  rb_log(log_debug, "%s.prepareForAccess: vmxSpec    = %s", class_name, vConnectParams.vmxSpec);
  rb_log(log_debug, "%s.prepareForAccess: credType   = %d", class_name, vConnectParams.credType);
    
  vIdentity = (NIL_P(rIdentity)) ? NULL : RSTRING_PTR(StringValue(rIdentity));

  rb_log(LOGLEVEL_VERBOSE, "%s.prepareForAccess: status=[starting] server=[%s], port=[%d], userid=[%s]", class_name, vConnectParams.serverName, vConnectParams.port, vConnectParams.creds.uid.userName);
    
  vixError = VixDiskLib_PrepareForAccess(&vConnectParams, vIdentity);
  if (VIX_FAILED(vixError))    {
    vix_check(vixError, "prepareForAccess");
  }
    
  rb_log(LOGLEVEL_VERBOSE, "%s.prepareForAccess: status=[complete]", class_name);
...
/*
 * The "endAccess" class method.
 */
static VALUE
vdl_endAccess(VALUE rSelf, VALUE rConnectParams, VALUE rIdentity) {
  VixDiskLibConnectParams vConnectParams;
  VixError                vixError;
  VALUE                   tval;
  const char             *vIdentity;
  ...
  rb_log(log_debug, "%s.endAccess: vmxSpec    = %s", class_name, vConnectParams.vmxSpec);
  rb_log(log_debug, "%s.endAccess: credType   = %d", class_name, vConnectParams.credType);
    
  vIdentity = (NIL_P(rIdentity)) ? NULL : RSTRING_PTR(StringValue(rIdentity));

  rb_log(LOGLEVEL_VERBOSE, "%s.endAccess: status=[starting] server=[%s], port=[%d], userid=[%s]", class_name, vConnectParams.serverName, vConnectParams.port, vConnectParams.creds.uid.userName);
    
  vixError = VixDiskLib_EndAccess(&vConnectParams, vIdentity);
  if (VIX_FAILED(vixError))    {
    vix_check(vixError, "endAccess");
  }
    
  rb_log(LOGLEVEL_VERBOSE, "%s.endAccess: status=[complete]", class_name);
...
void Init_VixDiskLib_raw()	{
  ...
  rb_define_singleton_method(cVixDiskLib_raw, "cleanup", 	           vdl_cleanup,              1);
	...
	rb_define_singleton_method(cVixDiskLib_raw, "endAccess",           vdl_endAccess,            2);
  ...
  rb_define_singleton_method(cVixDiskLib_raw, "initEx",              vdl_initEx,               5);
	rb_define_singleton_method(cVixDiskLib_raw, "isAttachPossible",    vdl_isAttachPossible,     2);
	...
	rb_define_singleton_method(cVixDiskLib_raw, "prepareForAccess",    vdl_prepareForAccess,     2);
  ...
  thumbPrint_sym          = STR2SYM("thumbPrint");            // :thumbPrint
  privateUse_sym          = STR2SYM("privateUse");            // :privateUse
