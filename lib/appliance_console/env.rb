...
module ApplianceConsole
  class Env
    ...
    def self.[](var)
      File.exists?(NET_FILE) ? `#{NET_FILE} -GET #{var.to_s.upcase}`.chomp : var
    end
    ...
    def self.[]=(var, *values)
      ...
      `#{NET_FILE} -#{var.to_s.upcase} #{values.join(" ")} 2> #{ERR_FILE}` if File.exists?(NET_FILE)
    end
    ...
    def self.clear_errors
      @changed = false
      File.delete(ERR_FILE) if File.exists?(ERR_FILE)
    end
