class MiqAeInstance < MiqAeBase
  ...
  validate :uniqueness_of_instance_name, :on => :create
  ...
  def set_field_attribute(field, value, attribute)
    ...
    val   = ae_values.detect { |v| v.field_id.casecmp(field.id) == 0 }
    val ||= build_value(:ae_field => field, :ae_instance => self, :name => field.name)
    val.send("#{attribute}=", value)
    save
  end

  def build_value(options)
    MiqAeValue.new(options).tap { |v| ae_values << v }
  end
  ...
  def uniqueness_of_instance_name
    return false unless ae_class
    unless name
      errors.add(:name, "instance name cannot be blank")
      return false
    end
    x = ae_class.ae_instances.detect { |i| i.name.casecmp(name) == 0 }
    return true unless x
    return true if x == self
    return false unless uniqueness_of_name
