class MiqAeYamlExport
  ...
  def write_namespace_file(ns_obj)
    _log.info("Exporting namespace: <#{ns_obj.fqname_from_objects}>")
  ...
  def write_method_file(method_obj, export_file_hash)
    ...
    method_obj.data += NEW_LINE if method_obj.data && !method_obj.data.end_with?(NEW_LINE)
  ...
  def add_to_manifest(base_path, export_hash)
    ...
    size = export_hash['export_data'] ? export_hash['export_data'].length : 0
    sha1 = export_hash['export_data'] ? Digest::SHA1.base64digest(export_hash['export_data']) : ''
