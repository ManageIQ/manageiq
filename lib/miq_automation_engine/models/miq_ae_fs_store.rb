module MiqAeFsStore
  ...
  module ClassMethods
    ...
    def tree_entries(git_repo, parent_dir, yaml_file, filters, overwrite_name = false)
      ...
      git_repo.nodes(parent_dir).collect do |node|
        ...
        partial_name = node[:name].split('.')[0] if node[:name].ends_with?(MiqAeConstants::CLASS_DIR_SUFFIX)
  ...
  def load_class_children(klass, sub_dir = nil, filter = '*')
    ...
    parent_dir = "#{parent_dir}#{MiqAeConstants::CLASS_DIR_SUFFIX}"
    ...
    git_repo.nodes(child_dir).each do |entry|
      next if entry[:name] == MiqAeConstants::CLASS_YAML_FILE
  ...
  def load_class_methods(filter = '*')
    load_class_children(MiqAeMethod, MiqAeConstants::METHODS_DIRECTORY, filter) +
      load_class_children(MiqAeMethod, "#{MiqAeConstants::METHODS_DIRECTORY}/#{MiqAeConstants::CLASS_SCOPE_PREFIX}", filter)
  end
