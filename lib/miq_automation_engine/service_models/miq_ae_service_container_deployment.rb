...
module MiqAeMethodService
  class MiqAeServiceContainerDeployment < MiqAeServiceModelBase
    ...
    RHEL_SUBSCRIPTION_FILE_NAME = "rhel_subscribe_inventory.yaml"
    ...
    def subscribe_cluster(deployment_master, user)
      perform_agent_commands(deployment_master, user, ["sudo mv ~/#{RHEL_SUBSCRIPTION_FILE_NAME} #{INVENTORIES_PATH}"])
      run_playbook_command(deployment_master, user, "ansible-playbook #{INVENTORIES_PATH}playbooks/byo/rhel_subscribe.yml -i "\
    ...
    def add_automation_task(task)
      object_send(:update_attributes!, :automation_task_id => task.id)
    end

    def customize(options = {})
      ar_method do
        @object.customizations = {:agent => {}} if @object.customizations.empty?
    ...
    def find_vm_by_type(tag)
      requests = MiqRequest.where(:id => automation_task.miq_request.options["#{tag}_request_ids"])
    ...
    def agent_pid
      ar_method do
        @object.customizations.fetch_path(:agent, :agent_pid)
      end
    end

    def agent_socket
      ar_method do
        @object.customizations.fetch_path(:agent, :agent_socket)
      end
    end

    def ansible_pid
      ar_method do
        @object.customizations.fetch_path(:agent, :ansible_pid)
      end
    end

    def ssh_auth
      object_send(:ssh_auth)
    end
    private :ssh_auth
  end
end
