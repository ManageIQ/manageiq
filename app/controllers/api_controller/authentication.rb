    ...
    class ApiController
      module Authentication
        ...
        def validate_system_token_server(server_guid)
          ...
          raise "Invalid server_guid #{server_guid} specified" if MiqServer.where(:guid => server_guid).blank?
        end

        def validate_system_token_timestamp(timestamp)
          raise "Missing timestamp" if timestamp.blank?
          raise "Invalid timestamp #{timestamp} specified" if (Time.now.utc - timestamp).abs > 300
        end
      end
    end
  end
