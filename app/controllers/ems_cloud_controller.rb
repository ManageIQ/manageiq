class EmsCloudController < ApplicationController
  ...
  def update_ems_button_cancel
    ...
    render :update do |page|
      if @lastaction == "show"
        page.redirect_to ems_cloud_path(update_ems, :flash_msg => _("Edit of %{model} \"%{name}\" was cancelled by the user") %
                             {:model => ui_lookup(:model => model_name), :name => update_ems.name})
      else
    ...
    case params[:button]
      when "add"
        if !@flash_array
          ...
          set_ems_record_vars(ems)
        end
        if ems.valid? && ems.save
          construct_edit_for_audit(ems)
          AuditEvent.success(build_created_audit(ems, @edit))
          render :update do |page|
            page.redirect_to :action=>'show_list', :flash_msg=>_("%{model} \"%{name}\" was saved") % {:model=>ui_lookup(:tables=>@table_name), :name=>ems.name}
          end
        else
          @in_a_form = true
  ...
  def set_ems_record_vars(ems, mode = nil)
    ...
    ems.tenant_id = params[:tenant_id] if ems.is_a?(EmsAzure)
