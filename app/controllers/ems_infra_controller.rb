class EmsInfraController < ApplicationController
  ...
  def scaledown
    ...
    if host_ids.nil?
      ...
      # figure out scaledown parameters
      stack_parameters = get_scaledown_parameters(hosts, @infra, @compute_hosts)

      return_message = (" Scaling down to %{a} compute nodes") % {:a => stack_parameters['ComputeCount']}
  ...
  def update_stack(stack, stack_parameters, provider_id, return_message)
    ...
    elsif stack_parameters.length > 0
      # A value was changed
      begin
  ...
  def verify_hosts_for_scaledown(hosts)
    ...
    hosts.each do |host|
      if not host.admin_disabled
        has_invalid_nodes = true
        error_return_message += _(" %{host_uid_ems} needs to be in maintenance mode before it can be removed ") % {:host_uid_ems => host.uid_ems}
      end
      if host.number_of(:vms) > 0
        has_invalid_nodes = true
        error_return_message += _(" %{host_uid_ems} needs to be evacuated before it can be removed ") % {:host_uid_ems => host.uid_ems}
      end
      if !host.name.include?('Compute')
        has_invalid_nodes = true
  ...
  def get_scaledown_parameters(hosts, provider, compute_hosts)
    ...
    host_physical_resource_ids = hosts.map {|host| host.ems_ref_obj}    
