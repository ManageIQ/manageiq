class MiqPolicyController < ApplicationController
  ...
  def replace_right_cell(nodetype, replace_trees = [], presenter = nil)
      ...
      presenter.update(:main_div,
        if @profiles
          ...
          right_cell_text = _("All %{typ} %{model}") % {
            :typ   => "#{ui_model_from_id(@sb[:nodeid])} #{@sb[:mode] ? @sb[:mode].capitalize : ""}",
    ...
    when 'p'
      ...
      model_name = ui_model_from_id(@sb[:nodeid])
      if @policy.id.blank?
  ...
  def folder_get_info(folder_node)
    ...
    if x_active_tree == :policy_tree
      if nodeid.nil? && ["compliance", "control"].include?(nodetype.split('-').last)
        ...
        @right_cell_text = _("All %{typ} %{model}") % {:typ   => ui_model_from_id(@sb[:nodeid]),
                                                       :model => ui_lookup(:models => "MiqPolicy")}
      ...
      @right_cell_text = _("All %{typ} %{model}") % {:typ => ui_model_from_id(@sb[:folder]), :model => ui_lookup(:models => "Condition")}
      ...
      @right_cell_text = _("All %{typ} %{model}") % {:typ => ui_model_from_id(@sb[:folder]), :model => ui_lookup(:models => "MiqAlertSet")}
