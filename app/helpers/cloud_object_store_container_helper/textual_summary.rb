module CloudObjectStoreContainerHelper::TextualSummary
  ...
  def textual_bytes
    {:label => "Bytes", :value => number_to_human_size(@record.bytes, :precision => 2)}
  end

  def textual_object_count
    @record.object_count
  end
  ...
  def textual_cloud_tenant
    ...
    if cloud_tenant && role_allows(:feature => "cloud_tenant_show")
      h[:title] = _("Show this Cloud Object Store Container's parent %{parent}") % {:parent => label}
  ...
  def textual_cloud_object_store_objects
    label = ui_lookup(:tables => "cloud_object_store_objects")
    ...
    if num > 0 && role_allows(:feature => "cloud_object_store_object_show_list")
      h[:title] = _("Show this Cloud Object Store Container's child %{children}") % {:children => label}
