class ApplicationHelper::Toolbar::EmsContainersCenter < ApplicationHelper::Toolbar::Basic
  button_group('ems_container_vmdb', [
    {
      :buttonSelect => "ems_container_vmdb_choice",
      :icon         => "fa fa-cog fa-lg",
      :title        => N_("Configuration"),
      :text         => N_("Configuration"),
      :items => [
        {
          :button       => "ems_container_refresh",
          :icon         => "icon fa fa-refresh fa-lg",
          :text         => N_("Refresh Items and Relationships"),
          :title        => N_("Refresh Items and Relationships for all \#{ui_lookup(:table=>\"ems_containers\")}"),
          :confirm      => N_("Refresh Items and Relationships related to  \#{ui_lookup(:table=>\"ems_containers\")}?"),
          :enabled      => "false",
          :url_parms    => "main_div",
          :onwhen       => "1+",
        },
        {
          :separator    => nil,
        },
        {
          :button       => "ems_container_new",
          :icon         => "pficon pficon-add-circle-o fa-lg",
          :url          => "/new",
          :text         => N_("Add a New \#{ui_lookup(:table=>\"ems_container\")}"),
          :title        => N_("Add a New \#{ui_lookup(:table=>\"ems_container\")}"),
        },
        {
          :button       => "ems_container_edit",
          :icon         => "pficon pficon-edit fa-lg",
          :text         => N_("Edit Selected \#{ui_lookup(:table=>\"ems_container\")}"),
          :title        => N_("Select a single \#{ui_lookup(:table=>\"ems_container\")} to edit"),
          :url_parms    => "main_div",
          :enabled      => "false",
          :onwhen       => "1",
        },
        {
          :button       => "ems_container_delete",
          :icon         => "pficon pficon-delete fa-lg",
          :text         => N_("Remove \#{ui_lookup(:tables=>\"ems_containers\")} from the VMDB"),
          :title        => N_("Remove selected \#{ui_lookup(:tables=>\"ems_containers\")} from the VMDB"),
          :url_parms    => "main_div",
          :confirm      => N_("Warning: The selected \#{ui_lookup(:tables=>\"ems_containers\")} and ALL of their components will be permanently removed from the Virtual Management Database.  Are you sure you want to remove the selected \#{ui_lookup(:tables=>\"ems_containers\")}?"),
          :enabled      => "false",
          :onwhen       => "1+",
        },
      ]
    },
  ])
  button_group('ems_container_policy', [
    {
      :buttonSelect => "ems_container_policy_choice",
      :icon         => "fa fa-shield fa-lg",
      :title        => N_("Policy"),
      :text         => N_("Policy"),
      :enabled      => "false",
      :onwhen       => "1+",
      :items => [
        {
          :button       => "ems_container_tag",
          :icon         => "pficon pficon-edit fa-lg",
          :text         => N_("Edit Tags"),
          :title        => N_("Edit Tags for this \#{ui_lookup(:table=>\"ems_container\")}"),
          :url_parms    => "main_div",
          :enabled      => "false",
          :onwhen       => "1+",
        },
      ]
    },
  ])
end
