    ...
    select(
      ...
      :items => [
        ...
        button(
          ...
          t = proc do
            _('Copy this Condition to a new Condition assigned to Policy [%{condition_policy_description}]') %
              {:condition_policy_description => @condition_policy.description}
          end,
          t,
          :url_parms => "?copy=true"),
        button(
        ...
        button(
          ...
          'pficon pficon-delete fa-lg',
          t = proc do
            _('Remove this Condition from Policy [%{condition_policy_description}]') % {:condition_policy_description => @condition_policy.description}
          end,
          t,
          :url_parms => "?policy_id=\#{@condition_policy.id}",
          :klass     => ApplicationHelper::Button::ReadOnly,
          :confirm   => proc { _("Are you sure you want to remove this Condition from Policy [%{condition_policy_description}]?") % {:condition_policy_description => @condition_policy.description} }),
      ]
    ),
  ])
end
