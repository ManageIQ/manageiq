...
module RefreshParser
  ...
  def self.ems_inv_to_hashes(inv)
    ...
    result[:hosts], uids[:hosts], uids[:clusters_by_host], uids[:lans], uids[:switches], uids[:guest_devices], uids[:scsi_luns] = self.host_inv_to_hashes(inv[:host], inv, uids[:storages], uids[:clusters])
    result[:vms], uids[:vms] = self.vm_inv_to_hashes(inv[:vm], inv[:storage], uids[:storages], uids[:hosts], uids[:clusters_by_host], uids[:lans])
    ...
    result[:customization_specs] = self.customization_spec_inv_to_hashes(inv[:customization_specs]) if inv.has_key?(:customization_specs)
  ...
  def self.storage_inv_to_hashes(inv)
    ...
    inv.each do |mor, storage_inv|
      ...
      loc = uid = self.normalize_storage_uid(storage_inv)
  ...
  def self.host_inv_to_hashes(inv, ems_inv, storage_uids, cluster_uids)
    ...
    inv.each do |mor, host_inv|
      ...
      # Get the IP address
      ipaddress = self.host_inv_to_ip(host_inv, hostname) || hostname
      ...
      if parent_type == :host_res
        rp_uids = self.get_mors(parent_data, "resourcePool")
      ...
      storages = self.get_mors(host_inv, 'datastore').collect { |s| storage_uids[s] }.compact
  ...
  def self.host_inv_to_ip(inv, hostname = nil)
    ...
    unless default_gw.blank?
      ...
      consoleVnics = inv.fetch_path("config", "network", "consoleVnic")
      vnics.concat(consoleVnics) unless consoleVnics == nil
  ...
  def self.vm_inv_to_hashes(inv, storage_inv, storage_uids, host_uids, cluster_uids_by_host, lan_uids)
    ...
    inv.each do |mor, vm_inv|
      ...
      new_result = {
        ...
        :template         => template,
        :linked_clone     => self.vm_inv_to_linked_clone(vm_inv),
        :fault_tolerance  => self.vm_inv_to_fault_tolerance(vm_inv),
        ...
        :storages          => storages,
        :storage           => storage_uids[:storage_id][self.normalize_vm_storage_uid(vm_inv, storage_inv)],
  ...
  def self.vm_inv_to_linked_clone(inv)
    self.vm_inv_to_shared(inv) && inv.fetch_path("summary", "config", "ftInfo", "instanceUuids").to_miq_a.length <= 1
  end
  ...
  def self.vm_inv_to_guest_device_hashes(inv, lan_uids)
    ...
    inv.to_miq_a.find_all { |d| d.has_key?('macAddress') }.each do |data|
  ...
  def self.vm_inv_to_disk_hashes(inv, storage_uids)
    ...
    inv.each do |device|
      ...
      if device_type == 'disk'
        ...
        new_result[:disk_type] = if backing.has_key?('compatibilityMode')
  ...
  def self.snapshot_inv_to_snapshot_hashes(inv, current, parent_uid=nil)
    ...
    inv['childSnapshotList'].to_miq_a.each do |child_snapshot_info|
      result += self.snapshot_inv_to_snapshot_hashes(child_snapshot_info, current, nh[:uid])
    end
  ...
  def self.folder_and_dc_inv_to_hashes(folder_inv, dc_inv)
    folder_result, folder_uids = self.folder_inv_to_hashes(folder_inv, false)
    dc_result, dc_uids = self.folder_inv_to_hashes(dc_inv, true)
  ...
  def self.folder_inv_to_hashes(inv, is_dc)
    ...
    inv.each do |mor, data|
      ...
      child_mors = if is_dc
        self.get_mors(data, 'hostFolder') + self.get_mors(data, 'vmFolder')
      else
        self.get_mors(data, 'childEntity')
      end
  ...
  def self.cluster_inv_to_hashes(inv)
    ...
    inv.each do |mor, data|
      ...
      new_result = {
        ...
        :child_uids       => self.get_mors(data, 'resourcePool')
      }
      result << new_result
      result_uids[mor] = new_result
    end
  ...
  def self.rp_inv_to_hashes(inv)
    ...
    inv.each do |mor, data|
      ...
      new_result = {
        ...
        :child_uids => self.get_mors(data, 'resourcePool') + self.get_mors(data, 'vm')
      }
      result << new_result
      result_uids[mor] = new_result
    end
    ...
    [:folders, :clusters, :resource_pools, :hosts].each do |parent_type|
      data[parent_type].each do |parent_data|
        ...
        child_uids.each do |child_uid|
          ...
          child_type, child_inv = self.inv_target_by_mor(child_uid, inv)
          if child_type == :host_res
            child_uid = self.get_mors(child_inv, 'host')[0]
            if child_uid.nil?
  ...
  def self.inv_target_by_mor(mor, inv)
    ...
    mor_type = self.get_mor_type(mor)
  ...
  def self.host_parent_resource(host_mor, inv)
    ...
    [:host_res, :cluster].each do |type|
      ...
      parent = parent_data.find { |mor, parent_inv| self.get_mors(parent_inv, 'host').include?(host_mor) }
  ...
  def self.reconfig_inv_to_hashes(inv)
    ...
    uids[:storages] = self.reconfig_storage_inv_to_hashes(inv[:storage])
    uids[:lans] = self.reconfig_host_inv_to_lan_hashes(inv[:host])
    result[:vms] = self.reconfig_vm_inv_to_hashes(inv[:vm], uids[:storages], uids[:lans])
  ...
  def self.reconfig_storage_inv_to_hashes(inv)
    ...
    inv.each do |mor, storage_inv|
      ...
      loc = self.normalize_storage_uid(storage_inv)
  ...
  def self.reconfig_vm_inv_to_hashes(inv, storage_uids, lan_uids)
    ...
    inv.each do |mor, vm_inv|
      ...
      hardware = self.vm_inv_to_hardware_hash(vm_inv)
      hardware[:disks] = self.vm_inv_to_disk_hashes(vm_inv, storage_uids)
      hardware[:guest_devices], = self.vm_inv_to_guest_device_hashes(vm_inv, lan_uids)
      uid = hardware[:bios]
