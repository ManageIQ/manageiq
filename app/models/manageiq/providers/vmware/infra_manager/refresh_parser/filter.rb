  ...
  module RefreshParser::Filter
    def find_relats_vc_data(filtered_data, mappings)
      ...
      [:host, :vm, :folder, :dc, :cluster, :rp].each do |type|
        ...
        vr[key] |= filtered_data[type].collect { |mor, _data| mappings[type][mor] } unless filtered_data[type].empty?
      ...
      filtered_data[:vm].each do |mor, v|
        ...
        unless filtered_data[:storage].empty?
          ...
          unless uid.blank?
            s_mor, = filtered_data[:storage].find { |_mor, s| RefreshParser.normalize_storage_uid(s) == uid }
