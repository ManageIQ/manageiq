class ManageIQ::Providers::Redhat::InfraManager < ManageIQ::Providers::InfraManager
  ...
  def connect(options = {})
    ...
    result =
      if version == 4
        self.class.raw_connect_v4(server, port, path, username, password, service)
      else
        self.class.raw_connect_v3(server, port, path, username, password, service)
      end

    # Copy the API path to the endpoints table:
    default_endpoint.path = version == 4 ? '/ovirt-engine/api' : result.api_path
  ...
  def with_provider_connection(options = {})
    ...
    begin
      ...
      # it is clused using the `close` method.
      begin
