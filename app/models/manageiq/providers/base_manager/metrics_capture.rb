class ManageIQ::Providers::BaseManager::MetricsCapture
  ...
  def perf_capture_queue_targets(targets, interval, start_time:, end_time:, parent: nil)
    if batch_size > 1
      if parent # send all of the hosts in a cluster in one message
        perf_capture_queue_target(targets, interval, :start_time => start_time, :end_time => end_time, :rollup => true)
      else # batch sends
        targets.each_slice(batch_size) do |targets_batch|
          perf_capture_queue_target(targets_batch, interval, :start_time => start_time, :end_time => end_time)
        end
      end
    else # send individual messages
      task = create_rollup_task_for_cluster(parent, targets) if parent
      perf_capture_queue_targets_individ(targets, interval, :start_time => start_time, :end_time => end_time, :task_id => task&.id)
    end
  end

  # standard one message per object to collect
  def perf_capture_queue_targets_individ(targets, interval, start_time:, end_time:, task_id: nil)
  ...
  def perf_capture_queue_target(target, interval_name, start_time:, end_time:, task_id: nil, rollup: false)
    ...
    # GOAL only use put/put_unless_exists and drop put_or_update
