...
class ManageIQ::Providers::Ovirt4::InfraManager::EventCatcher::Runner < ManageIQ::Providers::BaseManager::EventCatcher::Runner
  ...
  def monitor_events
    ...
    # Inform the base class that the event monitor is running:
    event_monitor_running

    # Locate the service that manages events:
    events_service = connection.system_service.events_service

    # The events provided by the server have an increasing numeric identifier. We need to
    # remember the highest value seen, in order to avoid processing the same event multiple
    # times.
    from = 0

    # The first time that the events are retrieved we want only the last event. The server
    # returns events ordered by id, so to get the last we just need to retrieve at most
    # one event.
    max = 1

    # Retrieve events in a loop, till stop is requested:
    @stop = false
    until @stop
      events = events_service.list(:from => from, :max => max)
      unless events.empty?
        events.each do |event|
          set_event_name(event) if event.name.nil?
        end
        @queue.enq events
        from = events.first.id.to_i + 1
      end
      max = nil
      sleep_poll_normal
    end
  ...
  def process_event(event)
    _log.info("#{log_prefix} event.id: #{event.id}")
    _log.info("#{log_prefix} event.code: #{event.code}")
    _log.info("#{log_prefix} event.description: #{event.description}")
    if filtered?(event)
      _log.info "#{log_prefix} Skipping caught event [#{event}]"
    else
      _log.info "#{log_prefix} Caught event [#{event}]"
      EmsEvent.add_queue('add_ovirt4', @cfg[:ems_id], event)
    end
  end

  def filtered?(event)
     # This should check if the event should be filtered, according to the 'blacklist' of event
     # names used by the provider.
     false
  end


  # Some versions of the engine don't provide the event name, so we need to add it explicitly.
  def self.set_event_name(event)
    event.name = EVENT_NAMES[event.code]
    if event.name.nil?
      logger.warn("#{name}.#{__method__} Unknown oVirt event #{event.code}: #{event.description}")
      event.name = 'UNKNOWN'
    end
  end
  ...
  # This hash maps event codes to names.
  EVENT_NAMES = {
    0    => 'UNASSIGNED',
    1    => 'VDC_START',
    2    => 'VDC_STOP',
