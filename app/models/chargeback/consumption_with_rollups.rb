class Chargeback
  class ConsumptionWithRollups < Consumption
    ...
    def initialize(metric_rollup_records, start_time, end_time)
      ...
      @rate_cols = ChargeableField.cols_on_metric_rollup
      metric_rollup_records = metric_rollup_records.pluck(*@rate_cols) if metric_rollup_records.first.class <= MetricRollup
    ...
    def tag_names
      @tag_names ||= @rollups.inject([]) do |memo, rollup|
        rollup_value = resource_tag_names(rollup) | first_metric_rollup_record.resource_current_tag_names
        memo |= rollup_value if rollup_value.present?
        memo
      end
    end

    def tag_prefix
      klass_prefix = case first_metric_rollup_record.resource_type
                     when Container.name        then 'container_image'
                     when VmOrTemplate.name     then 'vm'
                     when ContainerProject.name then 'container_project'
                     end

      klass_prefix + Metric::ChargebackHelper::TAG_MANAGED_PREFIX
    end
    ...
    def tag_list_with_prefix
      if resource.kind_of?(Container)
        ...
        return "#{image_tag_name}#{first_metric_rollup_record.tag_names}".split("|").reject(&:empty?).map { |x| "#{tag_prefix}#{x}" } + (labels || [])
      end

      tag_names.map { |x| "#{tag_prefix}#{x}" } if tag_names
    end
    ...
    def sum_of_maxes_from_grouped_values(metric, sub_metric = nil)
      ...
      @grouped_values ||= {}
      grouped_rollups = @rollups.group_by { |x| x[ChargeableField.col_index('resource_id')] }
      @grouped_values[metric] ||= grouped_rollups.map do |_, rollups|
        rollups.collect { |x| x[ChargeableField.col_index(metric)] }.compact.max
      end.compact.sum
    end
    ...
    def chargeback_fields_present
      @chargeback_fields_present ||= @rollups.count{ |rollup| chargeback_fields_present?(rollup) }
    end
    ...
    def metering_used_fields_present
      @rollups.count{ |rollup| metering_used_fields_present?(rollup) }
    end
    ...
    def values(metric, sub_metric)
      @values ||= {}
      @values["#{metric}#{sub_metric}"] ||= begin
        sub_metric ? sub_metric_rollups(sub_metric) : @rollups.collect { |x| x[ChargeableField.col_index(metric)] }.compact
      end
    end

    def first_metric_rollup_record
      first_rollup_id = @rollups.first[ChargeableField.col_index('id')]
