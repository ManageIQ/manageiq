class TransformationMappingItem < ApplicationRecord
  ...
  validate :destination_datastore, :if => -> { destination_type.casecmp?('Storage') || destination_type.casecmp?('CloudVolumeType') }

  validate :source_datastore, :if => -> { source_type.casecmp?('Storage') }

  validate :destination_network, :if => -> { source_type.casecmp?('Lan') || source_type.casecmp?('CloudNetwork') }
