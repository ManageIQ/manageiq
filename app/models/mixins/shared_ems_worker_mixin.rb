module SharedEmsWorkerMixin
  ...
  module ClassMethods
    ...
    def desired_queue_names
      ...
      all_valid_ems_in_zone.collect do |e|
        queue_name_for_ems(e)
      end
    end
    ...
    def start_worker_for_ems(ems_or_queue_name)
      _log.info("Start worker for ems #{ems_or_queue_name.inspect}")
      # @gregB This takes the queue_name structure of either ems_1 or ems_1_2
      # and only returns the parent manager that we want to run aka ems_1 aka cloud manager
      ems_id = ems_or_queue_name.rpartition('_').last
      _log.info("EMS_ID is #{ems_id}")
      ems = ems_from_queue_name("ems_#{ems_id}")
      _log.info("EMS is #{ems.inspect} parent_ems_id is #{ems.parent_ems_id.nil?}")
      if ems.parent_ems_id.nil?
