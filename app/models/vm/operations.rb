module Vm::Operations
  ...
  included do
    supports :vnc_console do
      ...
      if vendor == 'vmware' && !(ext_management_system.present? && console_supports_type?('VNC'))
    ...
    supports :cockpit_console do
      canned_message = _('The web-based console is not available because the')
      unsupported_reason_add(:cockpit_console, _("#{canned_message} 'Cockpit' role is not enabled")) unless MiqRegion.my_region.role_active?('cockpit_ws')
      unsupported_reason_add(:cockpit_console, _("#{canned_message} Windows platform is not supported")) if platform.downcase == 'windows'
    end
    ...
    supports :launch_cockpit do
      ...
      # TODO: - THIS BELONGS ON CONTAINER GROUP!!!!!!
      if respond_to?(:ready_condition_status) && ready_condition_status != 'True'
        unsupported_reason_add(:launch_cockpit, 'The web-based console is not available because the Container Node is not powered on')
      end
    end
