module HttpdOauth2
  class HttpdOauth2Error < StandardError; end
  ...
  def authenticate_with_jwt(request)
    ...
    raise HttpdOauth2Error, "Failed to Authenticate with JWT - error #{e}"
  end

  def oidc_authenticate(username, password, request)
  ...
  def oidc_metadata_url_endpoint(oidc_param, metadata_url_key)
    ...
    raise HttpdOauth2Error, "Invalid #{HTTPD_OPENIDC_CONF} configuration, missing #{oidc_param} or OIDCProviderMetadataURL #{metadata_url_key} entry" if endpoint.blank?
  ...
  def get_jwt_token_from_oidc(username, password)
    ...
    raise HttpdOauth2Error, parsed_response["error_description"] if parsed_response["error"].present?

    parsed_response["access_token"]
  rescue => e
    raise HttpdOauth2Error, "Failed to get a JWT Token for user #{username} - error #{e}"
  end
  ...
  def introspect_jwt_token(jwt_token)
    ...
    raise HttpdOauth2Error, "Invalid access token, JWT is inactive" if parsed_response["active"] != true

    parsed_response
  rescue => e
    raise HttpdOauth2Error, "Failed to Validate the JWT - error #{e}"
  end
