miqHttpInject(angular.module('miq.containers.providersModule')).controller('containers.deployProviderDetailsCreateVMsController',
  ...
  function($rootScope, $scope, $timeout, $document) {
    ...
    $scope.reviewTemplate = "/static/deploy-provider-master-nodes-review-create-vms.html";
    var firstShow = true;
    ...
    $scope.applyMasterTemplate = function () {
      ...
      $scope.data.createMastersMemory = $scope.data.masterCreationTemplate.memo;
    ...
    $scope.applyNodeTemplate = function () {
      ...
      $scope.data.createNodesMemory = $scope.data.nodeCreationTemplate.memo;
