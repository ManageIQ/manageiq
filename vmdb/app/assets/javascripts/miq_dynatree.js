...
function cfmeOnClick_RoleFeatureCheck(node) {
  // If node was selected, now unchecking
...
function cfmeOnDblClick_Expand(node, event) {
  var exp = !Boolean(node.isExpanded());
  node.expand(exp);
...
function cfme_expand_parent_nodes(treename, selected_nodea) {
  var node = $("#" + treename + "box").dynatree("getTree").getNodeByKey(selected_node);
  node.makeVisible();
}

function cfme_dynatree_node_add_class(treename, key, klassa) {
...
function cfme_dynatree_redraw(treename) {
  $("#" + treename + "box").dynatree('getTree').redraw();
}
...
function miqOnMouseIn_HostNet(id) {
  var nid = hover_node_id(id);
  // div id exists
  if (nid) {
    var node = $('#' + id);
    var top = getAbsoluteTop(node);
    // Set quad top location
    $("#" + nid).css({top: (top - 220) + "px"});
    // Show the quad div
    $("#" + nid).show();
...
function hover_node_id(id) {
  var ids = id.split('_');
  var nid = ids[ids.length - 1];
  // Check for VM node or Host node
  return ((nid.split('-')[0] == 'v' ||
    nid.split('-')[0] == 'h') &&
...
function miqOnClick_HostNet(id) {
  ...
  var nid = ids[ids.length - 1].split('-');
  switch (nid[0]) {
    case 'v':
      DoNav("/vm/show/" + nid[1]);
      break;
