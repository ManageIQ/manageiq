...
function miqCheck_AE_All(button_div, gridname) {
  ...
  if (typeof ns_list_grid != "undefined" && gridname == "ns_list_grid") {
    ...
    class_methods_grid.checkAll(Boolean(state));
...
function miqInitGrids() {
  if (typeof miq_grids != "undefined") {
    $.each(miq_grids, function (key) {
      // pass they key (grid name), called function will get the grid hash
      miqInitGrid(key);
    });
  }
}
...
function miqInitGrid(grid_name) {
  // Get the hash for the passed in grid
  var grid_hash = miq_grids[grid_name];
  // Keep track of the grid checkboxes
  var miq_grid_checks = "";
  ...
  if (options.autosize) {
    ...
    // IE fix to eliminate scroll bars on initial display
  ...
  if (options.row_url &&
      options.row_url.lastIndexOf("_none_") != (options.row_url.length - 6)) {
  ...
  if (options.save_col_widths) {
    // Method called when resize starts
    grid.attachEvent("onResize", miqResizeCol);
    // Medhod called when resize ends
    grid.attachEvent("onResizeEnd", miqResizeColEnd);
    // Save the original column widths
    miq_grid_col_widths = grid.cellWidthPX.join(",");
  }
...
function miqDhtmlxgridSerialize(gridObj) {
  ...
  dhtmlxgridXml += "<rows>";
  var rowIds = gridObj.getAllRowIds().split(',');
