...
#         AuditEvent.success(build_saved_vm_audit(@record))
  ...
  def tree_select
    ...
    if x_tree[:type].to_s =~ /filter$/ && # FIXME: create some property on trees for this
  ...
  def replace_right_cell(action=nil)
    ...
    unless x_active_tree == :vandt_tree
      ...
      if params[:action] == "x_show" &&
         ...
         x_tree[:type].to_s !~ /filter$/       # Not in a filter tree; FIXME: create some property on trees for this
