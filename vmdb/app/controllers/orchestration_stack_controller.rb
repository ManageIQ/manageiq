class OrchestrationStackController < ApplicationController
          ...
          if @refresh_partial != nil
            if @refresh_div == "flash_msg_div"
              ...
              if ["instances"].include?(@display) # If displaying vms, action_url s/b show
                ...
                page.replace_html("main_div", :partial=>"layouts/gtl", :locals=>{:action_url=>"show/#{@orchestration_stack.id}"})
              else
  ...
  def show_association(action, display_name, listicon, method, klass, association = nil)
    ...
    unless params[:show].nil?
      ...
      drop_breadcrumb( { :name => "#{@orchestration_stack.name} (#{display_name})", :url=>"/orchestration_stack/#{action}/#{@orchestration_stack.id}?page=#{@current_page}"} )
      drop_breadcrumb( { :name => @item.name,                        :url=>"/orchestration_stack/#{action}/#{@orchestration_stack.id}?show=#{@item.id}"} )
      show_item
    else
      drop_breadcrumb( { :name => @orchestration_stack.name,                        :url=>"/orchestration_stack/show/#{@orchestration_stack.id}"}, true )
      drop_breadcrumb( { :name => "#{@orchestration_stack.name} (#{display_name})", :url=>"/orchestration_stack/#{action}/#{@orchestration_stack.id}"} )
      @listicon = listicon
      if association.nil?
        show_details(klass)
      else
