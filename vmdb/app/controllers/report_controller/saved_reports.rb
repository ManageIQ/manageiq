module ReportController::SavedReports
  ...
  def fetch_saved_report(id)
    ...
    if user.admin_user? || rr.miq_group_id == session[:group]
      ...
      if rr.status.downcase == "finished"
        ...
        if @report.blank?
          add_flash(I18n.t("flash.report.not_found_schedule_failed",
                          :name=>format_timezone(rr.created_on,Time.zone,"gtl")),
                    :error)
  ...
  def saved_report_delete
    ...
    if savedreports.empty? && params[:id].present? && !MiqReportResult.exists?(params[:id].to_i)
      ...
      add_flash(I18n.t("flash.record.no_longer_exists", :model=>"Saved Report"), :error)
    else
      ...
      add_flash(I18n.t("flash.selected_record_deleted", :model=>"Saved Report")) if @flash_array.nil?
    end
...
#   @embedded = true
    ...
    @right_cell_text    = I18n.t("cell_header.all_model_records", :model=>"Saved Reports")
