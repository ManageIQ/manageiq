class Authenticate
  ...
  def mode(username)
    username == "admin" ? "database" : config[:mode]
  end
  ...
  def authenticate_amazon(username, password)
    ...
    if amazon_auth.iam_authenticate(username, password)
      ...
      if config[:amazon_role] == true
        user = authorize_amazon_queue(username)
      else
  ...
  def authorize_amazon_queue(username)
    ...
    unless MiqEnvironment::Process.is_ui_worker_via_command_line?
      ...
      authorize_amazon(task.id, username)
    end

    task.id
  end

  def self.authorize_amazon(config, taskid, username)
    new(config).authorize_amazon(taskid, username)
  end

  def authorize_amazon(taskid, username)
  ...
  def authenticate_ldap(username, password)
    ...
    if ldap.bind(fq_user, password)
      ...
      if config[:ldap_role] == true
        user = authorize_ldap_queue(fq_user)
      else
  ...
  def authorize_ldap_queue(fq_user)
    ...
    unless MiqEnvironment::Process.is_ui_worker_via_command_line?
      ...
      authorize_ldap(task.id, fq_user)
    end

    task.id
  end

  def self.authorize_ldap(config, taskid, fq_user)
    new(config).authorize_amazon(taskid, fq_user)
  end

  def authorize_ldap(taskid, fq_user)
  ...
  def authenticate_httpd(username, request)
  ...
  def authorize_httpd_queue(username, request)
  ...
  def self.authorize_httpd(config, taskid, username, user_attrs, membership_list)
    new(config).authorize_httpd(taskid, username, user_attrs, membership_list)
  end

  def authorize_httpd(taskid, username, user_attrs, membership_list)
