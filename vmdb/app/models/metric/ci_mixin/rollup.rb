module Metric::CiMixin::Rollup
  def perf_rollup_to_parents(interval_name, start_time, end_time = nil)
    ...
    parents = parent_rollups.to_a.flat_map { |p| [p, interval_name] }
    ...
    parents.each_slice(2) do |parent, new_interval|
      ...
      when 'hourly', 'historical' then
        ...
        log_header = "MIQ(#{self.class.name}.perf_rollup_to_parents) Queueing [#{new_interval}] rollup to #{parent.class.name} id: [#{parent.id}] for times: #{times.inspect}"
        $log.info "#{log_header}..."
      ...
      when 'daily' then
        ...
        times_by_tp.each do |tp, times|
          log_header = "MIQ(#{self.class.name}.perf_rollup_to_parents) Queueing [#{new_interval}] rollup to #{parent.class.name} id: [#{parent.id}] in time profile: [#{tp.description}] for times: #{times.inspect}"
          $log.info "#{log_header}..."
