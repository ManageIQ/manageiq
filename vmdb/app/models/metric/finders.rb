module Metric::Finders
  def self.find_all_by_hour(resource, hour, interval_name)
    start_time, end_time = self.hour_to_range(hour)
    return self.find_all_by_range(resource, start_time, end_time, interval_name)
  end

  def self.find_all_by_day(resource, day, interval_name, time_profile)
    start_time, end_time = self.day_to_range(day, time_profile)
    return self.find_all_by_range(resource, start_time, end_time, interval_name)
  end
  ...
  def self.hash_by_capture_interval_name_and_timestamp(resource, start_time, end_time, interval_name)
    ...
    return self.find_all_by_range(resource, start_time, end_time, interval_name).each_with_object({}) do |p, h|
      if is_array
