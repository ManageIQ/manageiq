...
describe FixAuth::AuthModel do
  ...
  context "#configurations" do
    ...
    it "should only find vmdb records" do
      other_context
      bad_context
      expect(contenders).not_to include(other_context.typ)
      expect(contenders).to include(bad_context.typ)
    end

    it "should upgrade " do
      subject.fix_passwords(other_context)
      expect(other_context).to be_settings_changed
      new_settings = YAML.load(other_context.settings)
      expect(new_settings['production']['password']).to be_encrypted_version(2)
    end

    it "should fail on bad passwords" do
      expect { subject.fix_passwords(bad_context) }.to raise_error
    end

    it "should replace bad passwords" do
      subject.fix_passwords(bad_context, :invalid => "replacement")
      new_settings = YAML.load(bad_context.settings)
